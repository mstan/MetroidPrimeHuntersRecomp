...
param(
  ...
  # Deprecated: routine releases no longer require shard performance gate JSON.
  [string]$ShardPerformanceGate = '',
  # Deprecated: performance-gate staging is disabled by project validation policy.
...
if ($StageForShardPerformanceGate) {
  throw '-StageForShardPerformanceGate is disabled by project validation policy. Run only artifact packaging checks unless the user explicitly requests a performance gate.'
}
...
"@
  ...
  if ($shards.Count -ne 0) {
    ...
    if ($ShardPerformanceGate) {
      Write-Warning '-ShardPerformanceGate is ignored by project validation policy; performance gate JSON is not required or copied.'
    }
