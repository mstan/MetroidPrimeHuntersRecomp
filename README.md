...
Also new since v0.4.11: the v0.4.10 player-submitted multiplayer/performance
coverage is still promoted into compiled banks, and the build pins ndsrecomp's
MPH adventure culling and spawn-boundary fixes. Alpha diagnostics remain
default-on, so affected testers can send back coverage, performance, and
dispatch-miss logs from the same build.
...
gameplay keeps adaptive widescreen.
