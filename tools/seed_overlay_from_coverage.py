...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help="comma-separated entry kinds to emit. Defaults to all three for "
             "compatibility. Pass 'call,indirect' to apply the same policy "
             "tools/ingest_coverage_manifests.py uses for the main banks: a "
             "schema-4 manifest stores roots as a per-page BITMAP, and over a "
             "long session the interpreter re-enters at nearly every "
             "instruction of every hot function, so promoting them seeds a "
             "function start every few instructions and suppresses the "
             "superblock coalescing the dispatch cost depends on. Measured on "
             "MPH overlay 4 over 85 manifests: 8,368 call/indirect entries "
             "against 47,783 root observations.")
    ...
    kinds_ok = {k.strip() for k in args.kinds.split(",") if k.strip()}
    unknown = kinds_ok - GOOD_KINDS
    if unknown:
        raise SystemExit(f"--kinds: unknown kind(s) {sorted(unknown)}")
    ...
    lines = [
        "# AUTO-GENERATED by tools/seed_overlay_from_coverage.py; committed.",
        "# Addresses only -- the overlay image is decompressed from the ROM, so",
        "# no dumped bytes are involved and this file is safe to commit.",
